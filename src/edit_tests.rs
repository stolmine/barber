...
fn ripple_delete_middle() {
    ...
    assert_eq!(el.resolve(20), Some((30, 1.0, 0.0)));
    assert_eq!(el.resolve(19), Some((19, 1.0, 0.0)));
    assert_eq!(el.resolve(89), Some((99, 1.0, 0.0)));
...
fn ripple_delete_at_boundaries() {
    ...
    assert_eq!(el.resolve(0), Some((10, 1.0, 0.0)));

    let mut el2 = EditList::from_length(50);
    el2.fades_enabled = false;
    el2.ripple_delete(40, 50);
    assert_eq!(el2.total_frames(), 40);
    assert_eq!(el2.resolve(39), Some((39, 1.0, 0.0)));
...
fn crop_keeps_range() {
    ...
    assert_eq!(el.resolve(0), Some((10, 1.0, 0.0)));
    assert_eq!(el.resolve(29), Some((39, 1.0, 0.0)));
...
fn iter_source_frames_full() {
    ...
    el.fades_enabled = false;
    let frames: Vec<Option<(usize, f32, f32)>> = el.iter_source_frames(0, 5).collect();
    assert_eq!(frames, vec![Some((0, 1.0, 0.0)), Some((1, 1.0, 0.0)), Some((2, 1.0, 0.0)), Some((3, 1.0, 0.0)), Some((4, 1.0, 0.0))]);
}
...
fn iter_source_frames_after_delete() {
    ...
    el.fades_enabled = false;
    el.ripple_delete(20, 30);
    let frames: Vec<Option<(usize, f32, f32)>> = el.iter_source_frames(18, 5).collect();
    assert_eq!(frames, vec![Some((18, 1.0, 0.0)), Some((19, 1.0, 0.0)), Some((30, 1.0, 0.0)), Some((31, 1.0, 0.0)), Some((32, 1.0, 0.0))]);
}
...
fn iter_source_frames_partial() {
    ...
    el.fades_enabled = false;
    let frames: Vec<Option<(usize, f32, f32)>> = el.iter_source_frames(3, 4).collect();
    assert_eq!(frames, vec![Some((3, 1.0, 0.0)), Some((4, 1.0, 0.0)), Some((5, 1.0, 0.0)), Some((6, 1.0, 0.0))]);
}
...
fn gap_delete_replaces_with_silence() {
    ...
    assert_eq!(el.resolve(30), Some((30, 1.0, 0.0)));
    assert_eq!(el.resolve(99), Some((99, 1.0, 0.0)));
}
...
fn iter_source_frames_with_silence() {
    ...
    el.gap_delete(2, 4);
    let frames: Vec<Option<(usize, f32, f32)>> = el.iter_source_frames(0, 5).collect();
    assert_eq!(frames, vec![Some((0, 1.0, 0.0)), Some((1, 1.0, 0.0)), None, None, Some((4, 1.0, 0.0))]);
}
...
fn extract_regions_full() {
    ...
    if let RegionKind::Source { source_start, source_end } = regions[0].kind {
...
fn extract_regions_across_boundary() {
    ...
    if let RegionKind::Source { source_start, source_end } = regions[1].kind {
...
fn insert_at_start() {
    let mut el = EditList::from_length(100);
    el.fades_enabled = false;
    let patch = vec![Region::source(50, 60)];
    el.insert(0, &patch);
    assert_eq!(el.total_frames(), 110);
    assert_eq!(el.resolve(0), Some((50, 1.0, 0.0)));
    assert_eq!(el.resolve(9), Some((59, 1.0, 0.0)));
    assert_eq!(el.resolve(10), Some((0, 1.0, 0.0)));
}
...
fn insert_at_end() {
    let mut el = EditList::from_length(100);
    el.fades_enabled = false;
    let patch = vec![Region::source(0, 10)];
    el.insert(100, &patch);
    assert_eq!(el.total_frames(), 110);
    assert_eq!(el.resolve(99), Some((99, 1.0, 0.0)));
    assert_eq!(el.resolve(100), Some((0, 1.0, 0.0)));
    assert_eq!(el.resolve(109), Some((9, 1.0, 0.0)));
}
...
fn insert_in_middle() {
    let mut el = EditList::from_length(100);
    el.fades_enabled = false;
    let patch = vec![Region::silence(5)];
    el.insert(50, &patch);
    assert_eq!(el.total_frames(), 105);
    assert_eq!(el.resolve(49), Some((49, 1.0, 0.0)));
    assert_eq!(el.resolve(50), None);
    assert_eq!(el.resolve(54), None);
    assert_eq!(el.resolve(55), Some((50, 1.0, 0.0)));
    assert_eq!(el.resolve(104), Some((99, 1.0, 0.0)));
}
...
fn reverse_simple() {
    ...
    assert_eq!(el.resolve(0), Some((9, 1.0, 0.0)));
    assert_eq!(el.resolve(9), Some((0, 1.0, 0.0)));
    assert_eq!(el.resolve(5), Some((4, 1.0, 0.0)));
}
...
fn reverse_of_reverse_is_identity() {
    ...
    el.fades_enabled = false;
    let mut el_original_no_fade = el_original.clone();
    el_original_no_fade.fades_enabled = false;
    el.reverse_selection(10, 30);
    el.reverse_selection(10, 30);
    for i in 0..50 {
        assert_eq!(el.resolve(i), el_original_no_fade.resolve(i), "mismatch at frame {}", i);
    }
}
...
fn split_reversed_region() {
    ...
    el.crop(5, 15);
    assert_eq!(el.total_frames(), 10);
    assert_eq!(el.resolve(0), Some((14, 1.0, 0.0)));
    assert_eq!(el.resolve(9), Some((5, 1.0, 0.0)));
}
...
fn set_gain_range_applies_gain() {
    ...
    assert_eq!(el.total_frames(), 100);
    assert_eq!(el.resolve(10), Some((10, 1.0, 0.0)));
    assert_eq!(el.resolve(30), Some((30, 0.5, 0.0)));
    assert_eq!(el.resolve(60), Some((60, 1.0, 0.0)));
}
